...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            try:
                # Načti obsah souboru 'content.txt' při každém požadavku
                file_content = read_file(content_file)
                # Získej výstup příkazu `uptime -p`
                uptime = get_uptime()
                # Získej teplotu
                temperature = get_temperature()
                # Vlož obsah textového souboru, uptime a teplotu do HTML šablony
        ...
        if self.path == '/index.html':
            ...
            try:
                ...
                self.send_error(404, "File not found")
        elif self.path == '/video.h264':
            try:
                with open('/home/pi/WEB/video.h264', 'rb') as file:
                    self.send_response(200)
    ...
    def do_POST(self):
        ...
        if self.path == '/script_photo-hdr':
            ...
            try:
                result = subprocess.run(['/usr/bin/sudo', '/home/pi/WEB/video.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                if result.returncode != 0:
                    raise Exception(f"Error running script: {result.stderr}")
                self.send_response(303)
                self.send_header('Location', '/video.h264')
                self.end_headers()
            except Exception as e:
                self.send_error(500, f"Internal server error: {e}")
