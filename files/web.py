#!/usr/bin/python3
#
#  For V3 camera (camera.service is disabled by default and starts only on HTML page load because of temperature)
#
import io
import logging
import socketserver
...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def start_service(self):
        global service_started, service_timer
        if not service_started:
            print("Spuštění služby pomocí systemctl...")
            subprocess.run(["sudo", "systemctl", "start", "camera.service"])
            service_started = True

        if service_timer is not None:
            service_timer.cancel()
        service_timer = Timer(SHUTDOWN_INTERVAL, stop_service)
        service_timer.start()
...
def stop_service():
    ...
    if service_started:
        print("Zastavení služby pomocí systemctl...")
        subprocess.run(["sudo", "systemctl", "stop", "camera.service"])
        service_started = False  # Reset flagu po zastavení služby
